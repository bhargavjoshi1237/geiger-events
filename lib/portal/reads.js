...
import { includedSummary } from "@/lib/memberships/entitlements";
...
// What each of the member's orders may collect at the event — merch, swag,
// meal vouchers. Derived live from the allocation rules, so it reflects any
// change an organiser makes without the buyer refreshing anything.
//
// Keyed by order id. The RPC does no ownership check of its own (it isn't
// reachable by anon), so only ever pass orders already scoped to this member.
export async function listMemberEntitlements(orders) {
  const sb = adminClient();
  ...
  const entries = await Promise.all(
    rows.map(async (order) => {
      const { data, error } = await sb.rpc("issue_entitlements_for_order", {
        p_order: order.id,
      });
      if (error) {
        console.error("[portal.entitlements]", error.message);
        return [order.id, []];
      }
      return [
        order.id,
        (data || []).map((e) => ({
          ...
          itemId: e.item_id,
          name: e.variant_label ? `${e.item_name} — ${e.variant_label}` : e.item_name,
          imageUrl: e.image_url ?? "",
          issuance: e.issuance ?? "",
          periodMode: e.period_mode ?? "none",
          periodLabel: e.period_label ?? "",
          entitled: Number(e.entitled_qty ?? 0),
          collected: Number(e.redeemed_qty ?? 0),
          remaining: Number(e.remaining ?? 0),
          lastAt: e.last_at ?? null,
...
export async function listMemberMemberships(email) {
  ...
  return (data || []).map((m) => {
    ...
    return {
      ...
      // What the plan attaches (VOD, room access, event perks) — see
      // lib/memberships/entitlements.js.
      included: includedSummary(cfg),
