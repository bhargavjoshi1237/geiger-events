...
// staff = /checkin + /door, kiosk = /kiosk, issue = /issue (inventory hand-out).
// Separate code spaces, so one type's code can never open another's route.
const ROLE_TYPES = ["staff", "kiosk", "issue"];
const roleType = (value) => (ROLE_TYPES.includes(value) ? value : "staff");
...
function roleToRow(input) {
  ...
  if ("type" in input) row.type = roleType(input.type);
...
// type } or null (invalid / not configured). `type` ("staff" | "kiosk") scopes
// the lookup so the two code spaces stay separate; omit to match either.
export async function validateCheckinCode(eventId, code, type) {
