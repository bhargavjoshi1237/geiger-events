...
The point is to move **output token volume** off Codex, not to outsource judgment.
...
- Follow `AGENTS.md`, `MODULE_CONVENTIONS.md`, `SUPABASE_CONVENTIONS.md`,
...
opencode reads `AGENTS.md` and `AGENTS.md` natively, so don't paste the whole
...
Do not run a build (per `AGENTS.md`) and do not do browser testing — that is
