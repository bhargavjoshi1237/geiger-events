...
export function classifyPalette(extraction) {
  ...
  // A fill read straight off a `.card` rule beats both a named variable and a
  // derived value — it is the surface the site actually paints panels with —
  // but it still has to sit near this background and on the right side of it.
  ...
  // Button hover fill. A hover is a *shade* of the brand, so it has to share
  // the brand's hue and stay near its lightness — sites carry `:hover` rules for
  // every button they own, and without this the page picks up a stray black or
  // white and flashes it under the cursor. (github.com and laracon.us both do.)
...
export function buildThemePatch(extraction, options = {}) {
  ...
  if (apply.header) {
    ...
    if (links.length || cta || hasStyle) {
      patch.header = {
        ...
        // A pinned bar with a fill is what the source's own header does; a bar
        // that pins without one would smear the page through itself, so the
        // background only carries over when we actually read one.
  ...
  if (apply.footer) {
    ...
    // The footer slab's own colors live on the theme; its links live on
    // design.footer, which is why this half is a patch and that half is not.
    // A "slab" the same color as the page is not a slab — smashingmagazine.com
    // reports #ffffff on a #ffffff page — so it only carries over when it
    // actually reads as a separate band.
    ...
    if (fs.background && (!pageBg || contrast(fs.background, pageBg) >= 1.25)) {
      patch.footerStyle = {
        ...
        // Only carry their footer text color when it reads on their footer fill
        // — plenty of sites set `color` on a footer child, not the slab.
  ...
  if (apply.content) {
    ...
    // What the source tells mobile browsers to tint their chrome with.
