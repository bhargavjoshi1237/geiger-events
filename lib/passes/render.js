...
export function passSvg(template = {}, ctx = {}, opts = {}) {
  // A layout with an empty element list is a deliberately cleared card, not a
  // legacy design — only a design with no layout at all falls back to autoflow.
  if (opts.side === "back" || Array.isArray(template.layout?.elements)) {
