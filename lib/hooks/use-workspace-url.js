...
//
// Two navigation modes, see `apply()`: moving the project/tab goes through the
// App Router (the path feeds `useParams`), while opening an event/section/record
// only rewrites the query string via the History API — no RSC round-trip.
//
...
// Optional in-memory stand-in for the routed state, supplied by embedded shells
// that run outside a /project/<id> route (the landing playground). When a
// provider is present every consumer transparently reads/writes that state
// instead of the URL, so screens navigate inside the embed without moving the
// host page. See components/landing/playground_workspace_url.jsx.
...
export function useWorkspaceUrl() {
  ...
  // The open record in any config-driven record area (e.g. Conference modules).
  // One generic param shared by every such screen — only one is open at a time.
  ...
  // Navigate to the patched URL. Moving the project or the tab changes the path
  // segment that `useParams` derives the tab from, so it has to go through the
  // router. Everything else only moves the query string, and this whole route is
  // client-rendered — an RSC round-trip would re-render nothing while costing a
  // network hop in production, so those go through the History API instead,
  // which `useSearchParams` still picks up. `replace` skips the history entry.
  const apply = useCallback(
    (next, { replace = false } = {}) => {
      ...
      // Built off the live pathname because `pushState` takes a raw URL and,
      // unlike `router.push`, won't prefix the production basePath for us.
  ...
  // Replaces rather than pushes, so Back leaves the entity instead of stepping
  // back through every editor section the user visited.
  ...
  // Switch to another tab and open a workflow there in one navigation — used when
  // a different screen (e.g. Workflow Templates) hands off to the builder.
  ...
  // Open / close a record in any config-driven record area (Conference modules).
  // Opening resets to its default section (mirrors openEvent/openVenue).
  ...
  // Every hook above still runs (rules of hooks); an embedded shell just gets
  // its in-memory state back instead of the routed one.
