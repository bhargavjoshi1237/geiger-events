...
// A horseshoe leaves the top 120 degrees open for the stage.
const HORSESHOE_FROM = (11 * Math.PI) / 6;
const HORSESHOE_TO = (7 * Math.PI) / 6;
...
function roundedRectPath(cx, cy, rx, ry, r) {
  ...
  return (u) => {
    ...
    switch (i) {
      ...
      case 1: return arc(cx + ax, cy + ay, 0, s); // bottom-right corner
      case 2: return { x: cx + ax - s, y: cy + ry }; // bottom
      case 3: return arc(cx - ax, cy + ay, Math.PI / 2, s); // bottom-left corner
      case 4: return { x: cx - rx, y: cy + ay - s }; // left, upward
      case 5: return arc(cx - ax, cy - ay, Math.PI, s); // top-left corner
      case 6: return { x: cx - ax + s, y: cy - ry }; // top
      case 7: return arc(cx + ax, cy - ay, -Math.PI / 2, s); // top-right corner
