...
> **This product's schema is `events`.** Its migration ledger is
> `events.geiger_migrations`. Substitute `events` wherever this doc writes
...
(`events.tickets`), never `events.flow_tickets`.
...
-- Add seat maps
--
-- Owns events.seat_maps. Free-form header: what this changes and why.
...
-- @down
drop table if exists events.seat_maps;
```
...
alter table events.tickets add column if not exists tier text;   -- back-fill older copies
create index if not exists tickets_event_idx on events.tickets (event_id);

drop trigger if exists tickets_touch_updated_at on events.tickets;
create trigger tickets_touch_updated_at
before update on events.tickets
for each row execute function events.touch_updated_at();

drop policy if exists tickets_demo_all on events.tickets;
create policy tickets_demo_all on events.tickets for all to anon, authenticated
...
```sql
alter table events.tickets enable row level security;
drop policy if exists tickets_demo_all on events.tickets;
create policy tickets_demo_all on events.tickets for all to anon, authenticated
  using (true) with check (true);
```
...
-- @no-transaction
-- @up
create index concurrently if not exists tickets_event_idx on events.tickets (event_id);
```
