...
## 1. How this project differs from geiger-flow
...
## 3. The reusable Supabase component
...
// Optional convenience: a pre-guarded client (null when unconfigured) so callers
// don't repeat the guard + createClient() dance.
...
feature-specific belongs in the area's `lib/supabase/<area>.js`, not here. All
tables live in `public` with the `flow_events`-style prefix; there is no
`.schema()` wrapper because the suite shares one `public` schema.

---

## 4. The data-access layer (`lib/supabase/<area>.js`)

One file per area. It is the **only** place that talks to that area's table
(`events.js` owns `public.flow_events`).
...
### Canonical example (condensed from `lib/supabase/events.js`)
...
## 5. The metadata expansion bag

`public.flow_events` carries a `metadata jsonb not null default '{}'::jsonb`
...
its own RLS (see `supabase/sqls/events.sql`).
...
## 8. New-area checklist

1. **SQL:** `supabase/sqls/<area>.sql` — table in `public` (`flow_<area>` prefix),
   indexes, `updated_at` trigger, `metadata jsonb` bag, RLS. Run `npm run db:push`.
2. **Shared client:** reuse `createClient()` + `isSupabaseConfigured()`; only add a
   new file under `supabase/components/` if you need a genuinely shared helper.
