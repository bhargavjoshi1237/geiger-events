...
// upstream supplies them. Every generator call takes the map's `aspect`,
// because the canvas is not square: without it a rotated section's chairs are
// laid out in a sheared space and drift away from the block that owns them.
...
// A map with no config at all keeps the original proscenium look: a stage strip
// across the top with the sections in front of it.
const LEGACY_STAGE = {
  shape: "stage",
  x: 35,
  y: 2,
...
// config.stage was the original stage marker; config.field is the bowl-era
// replacement that can also be a pitch, court, rink or ring anywhere on the
// canvas. Reading the old key forward means no migration and no broken maps.
export function normalizeField(config) {
  const field = config?.field;
  if (field && typeof field === "object") return { ...LEGACY_STAGE, ...field };
  ...
  if (stage && typeof stage === "object") {
    return {
      ...
      x: Number(stage.x ?? LEGACY_STAGE.x),
      y: Number(stage.y ?? LEGACY_STAGE.y),
      width: Number(stage.width ?? LEGACY_STAGE.width),
      height: Number(stage.height ?? LEGACY_STAGE.height),
    };
  }
  return { ...LEGACY_STAGE };
}

// The traced-over floor plan image, if one was uploaded. `path` is kept so the
// image can be removed from storage when it's replaced.
export function normalizeBackground(config) {
  const bg = config?.background;
  return {
    url: bg?.url || "",
    path: bg?.path || "",
    opacity: typeof bg?.opacity === "number" ? bg.opacity : 0.6,
  };
}
...
export function normalizeSeatMap(row) {
  ...
  return {
    ...
    config,
    aspect: config.aspect || "16/10",
    field: normalizeField(config),
...
// How many live seat assignments (sold, comped or blocked) hang off a section
// or a whole map. Rebuilding chairs DELETES them, and seat_assignments.seat_id
// cascades — so a rebuild on a map with sales would quietly destroy the tickets
// people bought. Every destructive rebuild below checks this first.
//
// Returns a count, or null when the read itself failed (treated as "don't
// touch it" by the callers).
export async function countLiveAssignments({ sectionId = null, seatMapId = null } = {}) {
  if (!sectionId && !seatMapId) return 0;
  if (!isSupabaseConfigured()) return 0;
  try {
    const sb = createClient();
    let query = sb
      .from(ASSIGNMENTS)
      .select("id, seat:seats!inner(section_id, seat_map_id)", { count: "exact", head: true })
      .is("released_at", null);
    query = sectionId
      ? query.eq("seat.section_id", sectionId)
      : query.eq("seat.seat_map_id", seatMapId);
    const { count, error } = await query;
    if (error) {
      console.error("[seat_maps.countLiveAssignments]", error.message);
      return null;
    }
    return count ?? 0;
  } catch (e) {
    console.error("[seat_maps.countLiveAssignments]", e);
    return null;
  }
}
...
//
// Returns the new seat count, `false` when the write fails, or the string
// "locked" when the section has live assignments and rebuilding would delete
// them. The screen decides what to say.
export async function regenerateSectionSeats(section, aspect = "16/10") {
  if (!section?.id || !isSupabaseConfigured()) return false;
  try {
    const live = await countLiveAssignments({ sectionId: section.id });
    if (live === null) return false;
    if (live > 0) return "locked";
...
// Rebuild every seated section's chairs from the geometry already on the map.
// Sections keep their positions; only the chairs move. This is the repair for a
// map whose seats were generated before the coordinate space was fixed — their
// blocks are fine but the chairs sit on a sheared grid.
//
// Returns { sections, seats }, `"locked"` when the map has live assignments a
// rebuild would delete, or null on failure.
export async function rebuildMapSeats(seatMapId, aspect = "16/10") {
  if (!seatMapId || !isSupabaseConfigured()) return null;
  try {
    const live = await countLiveAssignments({ seatMapId });
    if (live === null) return null;
    if (live > 0) return "locked";

    const sb = createClient();
    const sections = await listSections(seatMapId);
    if (!sections) return null;

    const seated = sections.filter((s) => s.kind !== "ga");
    if (seated.length === 0) return { sections: 0, seats: 0 };

    const { error: delError } = await sb
      .from(SEATS)
      .delete()
      .in("section_id", seated.map((s) => s.id));
    if (delError) {
      console.error("[seat_maps.rebuild]", delError.message);
      return null;
    }

    const rows = [];
    for (const section of seated) {
    ...
    for (let i = 0; i < rows.length; i += CHUNK) {
      const { error } = await sb.from(SEATS).insert(rows.slice(i, i + CHUNK));
      if (error) {
        console.error("[seat_maps.rebuild]", error.message);
        return { sections: seated.length, seats: i, partial: true };
      }
    }
    return { sections: seated.length, seats: rows.length };
  } catch (e) {
    console.error("[seat_maps.rebuild]", e);
    return null;
  }
}

// Write a whole generated bowl (lib/seating/bowl.js) in bulk: one insert for
// the sections, then one chunked insert for every chair. `replace` clears the
// map first, which is what "regenerate the bowl" means — the organiser has just
// chosen a different shape, so the old ring is meaningless.
//
// Returns { sections, seats }, `"locked"` when replacing would delete seats
// people have already been given, or null. Seats are inserted in chunks because
// a two-tier arena is comfortably tens of thousands of rows.
export async function createBowl(seatMapId, drafts, { replace = false, aspect = "16/10" } = {}) {
  if (!seatMapId || !drafts?.length || !isSupabaseConfigured()) return null;
  try {
    const sb = createClient();

    if (replace) {
      // Dropping the sections cascades all the way through seats to the
      // assignments hanging off them — so a map with live sales is off limits.
      const live = await countLiveAssignments({ seatMapId });
      if (live === null) return null;
      if (live > 0) return "locked";

      // Seats cascade from their section, so dropping the sections is enough.
      const { error } = await sb.from(SECTIONS).delete().eq("seat_map_id", seatMapId);
      if (error) {
        console.error("[seat_maps.createBowl]", error.message);
        return null;
      }
    }
    ...
    const { data: sections, error: sectionError } = await sb
      .from(SECTIONS)
      .insert(drafts.map((d) => toSectionRow({ ...d, seatMapId })))
      .select("*");
    if (sectionError) {
      console.error("[seat_maps.createBowl]", sectionError.message);
      return null;
    }

    const created = (sections || []).map(normalizeSection);
    const seatRows = [];
    ...
    for (let i = 0; i < seatRows.length; i += CHUNK) {
      const { error } = await sb.from(SEATS).insert(seatRows.slice(i, i + CHUNK));
      if (error) {
        console.error("[seat_maps.createBowl]", error.message);
        // The sections exist and some chairs landed; report what got in rather
        // than pretending nothing happened.
        return { sections: created.length, seats: i, partial: true };
      }
    }

    return { sections: created.length, seats: seatRows.length };
  } catch (e) {
    console.error("[seat_maps.createBowl]", e);
    return null;
  }
}
...
export async function importManifest(seatMapId, parsed, aspect = "16/10") {
  ...
  try {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      const grid = generateSeats(created, aspect);
