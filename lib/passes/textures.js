...
// Canvas sources for the 3D lanyard showcase: the badge holder's card texture
// and the woven strap. Both return a plain HTMLCanvasElement so this file stays
// free of any three.js import — the scene wraps them in a texture.
//
// The card artwork goes through the same `passSvg` the preview, the print sheet
// and the PNG exporter use, so the hanging card can never drift from what
// prints.

// Longest edge each face is rasterized at before it is composited into the
// atlas. A touch above the atlas region it lands in, so the downscale stays
// clean without making every edit pay for a 2K decode.
const FACE_PX = 1024;

// One repeat of the strap. Slightly longer than it is wide, which spaces the
// marks the way real webbing does — the mark takes about a third of the run and
// leaves the rest black. Oversampled so it stays crisp on a tall panel.
const RIBBON_TILE = { w: 224, h: 192 };

// Length-to-width ratio of one strap tile. The scene divides the strap's length
// by (band width × this) to work out how many times to repeat it without
// squashing the mark.
export const RIBBON_TILE_ASPECT = RIBBON_TILE.w / RIBBON_TILE.h;

// How the card mesh in models/tag.glb samples its texture. Both faces share one
// image: the front reads the left half, the back reads the right half and
// mirrored (so it stays the right way round when you turn the card over), and
// both use only the top ~76% of it. Measured off the model's own UVs — the
// numbers are the mesh's, not a choice.
//
// The pixel size is chosen so each half is undistorted at the card's real
// 0.716 : 1 proportions.
export const CARD_ATLAS = {
  width: 1280,
  height: 1184,
  // x/y/w/h as fractions of the image, top-left origin (glTF UV convention).
  faces: {
...
// The DPI that renders this stock at FACE_PX on its longest edge, so a portrait
// lanyard card and a landscape name badge both get the same texel density.
...
// The design's proportions rarely match the holder's, so each face is fitted
// inside its half without distortion and the surround is filled with the card's
// own background — for the usual solid-colour design that reads as one surface.
...
// Both card faces composited into the single atlas the model expects, or null
// when the design can't be rasterized (the showcase then keeps its plain card
// rather than breaking the scene). `ctx` is the usual { event, attendee,
// qrSettings } the renderer takes.
...
// The Geiger mark (public/logo1.svg) as raw path data. Kept here rather than
// fetched so the strap texture needs no network round-trip and stays crisp at
// any size — a canvas Path2D scales where a rasterized asset would not.
const MARK = {
  w: 467,
  h: 285,
  paths: [
...
// One repeating tile of the lanyard strap: the white Geiger mark on black. The
// band's UV runs `u` along its length, so the mark is laid down rotated a
// quarter-turn — it then reads upright while the strap hangs vertically.
export function ribbonTileCanvas() {
  ...
  // Woven edge: a hair lighter along both selvedges so the strap catches light
  // like webbing rather than reading as a flat black rectangle.
  ...
  // Local +x now runs across the strap, so the mark is sized off the strap's
  // width and repeats along its length.
