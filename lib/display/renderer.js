...
// Canvas-2D renderer for Display Boards. One draw function per slide type, plus
// a BoardPlayer that advances the queue on a clock.
//
// This module is deliberately the ONLY place a board is painted, because both
// outputs go through it: the live /display/<id> route mounts a canvas and runs
// the player, and the export records that same canvas through
// captureStream() + MediaRecorder. A slide can therefore never look different in
// the download than it does on the wall.
//
// Everything here is pure canvas — no React, no DOM beyond the <canvas> and the
// <img> elements used to decode board imagery.
...
// Truncate to fit `maxWidth`, appending an ellipsis. Returns the drawn string.
...
// Greedy word wrap, capped at `maxLines` (the last line is ellipsized).
...
// Shrink `size` until the text fits, so a long event name never overflows.
...
// Event name on the left, wall clock on the right, over a hairline rule. Shared
// by every schedule slide so the board reads as one surface.
...
// Big section heading under the header. Returns the y to continue from.
...
// A session row used by Now & Next and Up next by room.
...
function drawDayGridSlide(ctx, { slide, event, sessions, theme, now }) {
  ...
  // Time window, snapped to the hour either side of the day's real extent.
  ...
  // Session blocks, positioned by time and column.
...
// Paint one slide onto a 1920x1080 context. `images` maps a URL to a decoded
// HTMLImageElement (or null when it failed to load) — see preloadImages.
...
// crossOrigin="anonymous" matters: without it a drawn image taints the canvas
// and captureStream()/MediaRecorder throw on export. Supabase's public bucket
// serves permissive CORS, so this succeeds for board imagery; anything that
// doesn't resolves to null and the slide falls back to its no-image treatment.
...
// Every image URL a board needs, decoded up front so no slide paints a blank.
...
// Drives a board across a canvas: advances the queue on a clock, cross-fades
// between slides, and redraws every frame so the wall clock and "NOW" flag stay
// truthful. The same player backs the live route and the video export — export
// just records the canvas it paints.
export class BoardPlayer {
  constructor(canvas, options = {}) {
    ...
    // Playback is external state, so the player reports its own transitions
    // rather than leaving callers to mirror them.
  ...
  // Swap in fresh data without interrupting playback — the live route polls and
  // calls this so a re-published board updates on the wall without a reload.
  ...
  // Jump straight to a slide — used by the builder's preview when you select a
  // node on the canvas.
  seek(index) {
  ...
  frame(t) {
    ...
    // Cross-fade: the background is always opaque, only the slide content fades,
    // so the board never flashes to white between slides.
...
// Record exactly one pass of the board off an offscreen canvas and resolve a
// WebM blob. Runs the same BoardPlayer the wall does, so what downloads is what
// plays. `onProgress` receives 0..1.
export function exportBoardVideo({
  ...
  return new Promise((resolve, reject) => {
    ...
    player.onComplete = () => {
      // A trailing tick lets the recorder flush the final frames.
...
// Board slides show a human date; the app stores ISO strings.
