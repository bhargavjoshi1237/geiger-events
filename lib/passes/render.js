...
import { elementText, fontStack, sideBg, sideLayout } from "./layout";
import { effectsSvg } from "./effects";
...
export const PASS_FIELDS = {
  ...
  company: true,
  tier: false,
  // Why this person is here — Speaker, Crew, Press… Off for a plain attendee
  // badge, on for the role designs.
  role: false,
...
// legacy path below, so print/export/preview need no branching. `opts.side`
// picks which face is drawn; the finish/effects layer is card-level and sits
// over both.
function layoutSvg(template, ctx, opts) {
  ...
  const cardBg = sideBg(template, side);
  ...
  for (const el of layout.elements || []) {
    ...
    parts.push(elementSvg(el, ctx, { w, h, images, preview, cardBg }));
  }

  // The finish sits over the artwork but under the trim border, the way a
  // laminate or foil does on a real card.
  parts.push(
    effectsSvg(template.effects, {
      w,
      h,
      radius,
      uid: `fx-${String(template.id || "card").replace(/[^a-zA-Z0-9_-]/g, "")}-${side}`,
    }),
  );
...
export function passSvg(template = {}, ctx = {}, opts = {}) {
  ...
  // auto-flow below until they're opened in the editor and converted. The back
  // only ever exists as a free layout, so it never falls through.
  if (opts.side === "back" || template.layout?.elements?.length) {
    return layoutSvg(template, ctx, opts);
  }
...
function autoFlowSvg(template = {}, ctx = {}, opts = {}) {
  ...
  if (fields.role && attendee.role) {
    ...
    lines.push({
      ...
      fill: accent,
      gap: size * 1.6,
      value: String(attendee.role).toUpperCase(),
      pill: true,
    });
  }
  if (fields.name) {
...
// Which saved template prints for a given pass, most specific binding first: the
// pass role (Speaker, Crew, Press…), then the ticket tier, then the default,
// then the first. Roles and tier ids are per-event but their names are stable
// across events, so both bind by name.
export function resolveTemplate(templates, tier, role) {
  const list = Array.isArray(templates) ? templates.filter(Boolean) : [];
  if (!list.length) return null;

  const matchOn = (key, field) => {
    const needle = String(key || "").trim().toLowerCase();
    if (!needle) return null;
    return (
      list.find((t) =>
        (Array.isArray(t[field]) ? t[field] : []).some(
          (name) => String(name || "").trim().toLowerCase() === needle,
        ),
      ) || null
    );
  };
