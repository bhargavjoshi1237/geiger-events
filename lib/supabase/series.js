...
// `public.flow_event_series` table. The event<->series link itself lives on
// `flow_events.series_id` (owned by events.js), so membership reads/writes here
// delegate to that module — one table, one owner. Pure: validate,
...
// a time via the flow_series_merge_settings RPC.

const TABLE = "flow_event_series";
...
export async function mergeSeriesSettings(id, patch) {
  ...
  try {
    const sb = createClient();
    const { error } = await sb.rpc("flow_series_merge_settings", {
      p_id: id,
      p_patch: patch,
    });
    if (error) {
