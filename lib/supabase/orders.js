...
import { releaseOrderSeats } from "./seating";
import { releaseOrderBooths } from "./expo";
...
export async function buyTicket({
  ...
  accessCode = null,
  // Assigned seating: the specific chairs the buyer holds, plus the token that
  // proves the holds are theirs. When present the purchase routes through
  // buy_seats, which validates the holds, delegates the money/inventory work to
  // buy_ticket, and writes one seat_assignments row per seat — all atomically.
  seatIds = null,
  seatToken = null,
  // Exhibitor booths: the same arrangement as seats, routed through buy_booths.
  // A purchase is either seated or boothed, never both — an exhibitor buying a
  // stand and a delegate buying a chair are two different orders.
  boothIds = null,
  boothToken = null,
  ...
  try {
    ...
    // Record the tier id, not just its name: inventory entitlement matches on
    // the stable id, so renaming a tier can't strand a buyer's entitlement.
    if (ticketId != null) meta.tierId = String(ticketId);
    ...
    // Seated purchases go through buy_seats, which takes the same arguments
    // minus p_qty (quantity is the number of seats) plus the seat ids/token.
    // A Stripe return trip passes only the token — seat ids would exceed
    // Stripe's 500-char metadata cap on a big block, so buy_seats resolves them
    // from the live holds instead.
    const seated = (Array.isArray(seatIds) && seatIds.length > 0) || Boolean(seatToken);
    // Booth purchases work identically through buy_booths — quantity is the
    // number of stalls, and the token resolves them on the Stripe return trip.
    const boothed =
      !seated && ((Array.isArray(boothIds) && boothIds.length > 0) || Boolean(boothToken));
    let data;
    let error;
    if (seated) {
      const { p_qty: _qty, ...seatArgs } = args;
      ({ data, error } = await sb.rpc("buy_seats", {
        ...seatArgs,
        p_seat_ids: Array.isArray(seatIds) ? seatIds : [],
        p_seat_token: seatToken || null,
      }));
    } else if (boothed) {
      const { p_qty: _qty, ...boothArgs } = args;
      ({ data, error } = await sb.rpc("buy_booths", {
        ...boothArgs,
        p_booth_ids: Array.isArray(boothIds) ? boothIds : [],
        p_booth_token: boothToken || null,
      }));
    } else {
      ({ data, error } = await sb.rpc("buy_ticket", args));
    }
    if (error) {
...
export async function cancelOrder(id, { actor = "" } = {}) {
  ...
  try {
    ...
    // Assigned seats and booths go back on sale. Both are no-ops for an order
    // that held neither.
    await releaseOrderSeats(id);
    await releaseOrderBooths(id);
    return true;
  } catch (e) {
...
// camelCase editor field → snake_case column, for the order edit screen.
...
// Correct an order's details after checkout (wrong email, a comped ticket, a
// Stripe reference a webhook missed). Only the keys present in `patch` are
// written; `note` is merged into the metadata bag, which several features
// share, so it is read-modify-written rather than overwritten. Refunds and
// cancellations deliberately aren't here — they go through issueRefund /
// cancelOrder so the ledger and timeline keep their entries.
export async function updateOrder(id, patch = {}) {
  if (!id || !isSupabaseConfigured()) return false;
  try {
    const sb = createClient();
    const row = {};
    for (const [key, column] of Object.entries(ORDER_EDIT_FIELDS)) {
      if (patch[key] === undefined) continue;
      if (key === "quantity") row[column] = Math.max(1, Math.round(Number(patch[key]) || 1));
      else if (key === "price" || key === "total") row[column] = Number(patch[key]) || 0;
      else row[column] = String(patch[key] ?? "");
    }
    if (!Object.keys(row).length && patch.note === undefined) return true;

    if (Object.keys(row).length) {
      const { error } = await sb.from("event_orders").update(row).eq("id", id);
      if (error) {
    ...
    if (patch.note !== undefined) {
      const { data, error: metaError } = await sb
        .from("event_orders")
        .select("metadata")
        .eq("id", id)
        .maybeSingle();
      if (metaError) {
        console.error("[orders.update.metadata]", metaError.message);
        return false;
      }
      const current =
      ...
      const { error: writeError } = await sb
        .from("event_orders")
        .update({ metadata: { ...current, note: patch.note } })
        .eq("id", id);
      if (writeError) {
        console.error("[orders.update.metadata]", writeError.message);
        return false;
      }
    }
    return true;
...
export function normalizeOrder(row) {
  ...
  return {
    ...
    // The organizer's private note, kept on the metadata bag so it never
    // reaches the buyer's receipt.
    note:
    ...
    stripePaymentDetails:
      row.stripe_payment_details && typeof row.stripe_payment_details === "object"
        ? row.stripe_payment_details
        : null,
