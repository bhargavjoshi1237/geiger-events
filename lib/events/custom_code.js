...
function clear(doc, scope) {
  if (!doc) return;
  doc.querySelectorAll(`[${MARK}="${scope}"]`).forEach((el) => el.remove());
}

function injectStyles(doc, customCode, scope) {
  const cc = normalizeCustomCode(customCode);
 ...
 */
function injectScripts(doc, customCode, scope, cancelled) {
  const cc = normalizeCustomCode(customCode);
  ...
  const next = (index) => {
    ...
    const script = doc.createElement("script");
    script.src = externals[index].url;
    script.async = false;
    if (externals[index].defer) script.defer = true;
    script.setAttribute(MARK, scope);
