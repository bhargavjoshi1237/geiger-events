...
// Core workspace permissions. Addon-contributed keys are appended below, so an
// addon's views and controls appear in the Roles & Permissions matrix without
// that screen knowing addons exist.
const CORE_PERMISSIONS = [
  ...
  { key: "view.orders", label: "Orders", group: "Workspace views" },
  { key: "view.inventory", label: "Inventory", group: "Workspace views" },
...
// A permission an addon declares is additive and de-duplicated — an addon may
// reuse a core key (e.g. settings.manage) without doubling it in the matrix.
export const WORKSPACE_PERMISSIONS = [
  ...CORE_PERMISSIONS,
  ...ADDON_PERMISSIONS.filter(
    (p) => !CORE_PERMISSIONS.some((c) => c.key === p.key),
  ),
];
