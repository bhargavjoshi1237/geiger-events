...
// An event's tickets (metadata.tickets entries) with each applied type's rules
// resolved, for the public storefront. Goes through the public_event_tickets
// SECURITY DEFINER RPC so anon buyers see rules that live in the member-only
// records table. Returns camelCase view models (or [] when none / not configured).
export async function listEventTicketsResolved(eventId) {
  if (!eventId || !isSupabaseConfigured()) return [];
  try {
    const sb = createClient();
    const { data, error } = await sb.rpc("public_event_tickets", {
      p_event_id: eventId,
    });
    if (error) {
      console.error("[ticketing.eventTickets]", error.message);
      return [];
    }
    ...
    console.error("[ticketing.eventTickets]", e);
    return [];
  }
}
