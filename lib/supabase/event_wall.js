...
export async function getWall(projectId) {
  ...
  try {
    ...
    const { data: created, error: createError } = await sb
      ...
      .single();
    if (createError) {
      // Lost a get-or-create race: a concurrent first-open already inserted this
      // project's wall (both slug and project_id are unique). Rather than
      // surface the duplicate-key error, re-select and return the winner's row.
      const { data: existing } = await sb
        .from(TABLE)
        .select("*")
        .eq("project_id", projectId)
        .maybeSingle();
      if (existing) return normalizeWall(existing);
...
// Read-only lookup by project. Unlike getWall() this never creates the row, so
// it is safe from public/anon surfaces (the event page reads the wall's logo as
// the organiser's fallback photo). Returns null when the project has no wall.
export async function getWallByProject(projectId) {
  if (!projectId || !isSupabaseConfigured()) return null;
  try {
    const sb = createClient();
    const { data, error } = await sb
      ...
      .maybeSingle();
    if (error) {
      console.error("[eventWall.getByProject]", error.message);
      return null;
    }
    return normalizeWall(data);
  } catch (e) {
    console.error("[eventWall.getByProject]", e);
    return null;
  }
}
