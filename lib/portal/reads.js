...
const EVENT_SELECT =
  "name, event_date, event_time, venue, address, city, timezone, cover_url, project_id";
...
function mapOrder(o) {
  ...
  return {
    ...
    eventId: o.event_id,
    projectId: o.event?.project_id ?? null,
...
// No email/session check — the order id itself is the bearer secret, same as
// the "Order ID" QR encoding option. Used only by the public ticket-QR image
// route so an emailed ticket renders without the recipient being signed in.
export async function getOrderPublic(orderId) {
  const sb = adminClient();
  ...
  const { data, error } = await sb
    ...
    .maybeSingle();
  if (error) {
    console.error("[portal.orderPublic]", error.message);
    return null;
  }
  return data ? mapOrder(data) : null;
}
