...
export function seedLayout(template = {}) {
  ...
  // The role plate — what a speaker/crew badge is read from across a room — sits
  // above the name so it stays legible when the card is half-covered.
  if (fields.role) {
    elements.push(
      newElement("text", {
        field: "role",
...
// --- Sides --------------------------------------------------------------------
// A card has a front and a back. The front stays `template.layout`/`template.bg`
// so every design saved before this keeps rendering untouched; the back is its
// own layout under `template.back`, empty until something is put on it.
...
// Ensure a template carries both sides. Idempotent, like withLayout.
...
// Whether the back is worth printing at all — an untouched back would otherwise
// waste a sheet per page.
...
// Every image URL a template references, across both sides — the exporter has to
// inline all of them before rasterising, since an SVG in an <img> can't fetch
// anything.
