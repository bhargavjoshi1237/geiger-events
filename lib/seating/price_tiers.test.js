...
test("more distinct prices than bands share the cheapest colour", () => {
  // Sized off the scale, not a literal: lengthening PRICE_SCALE must not turn
  // this into a test of nothing.
  const count = PRICE_SCALE.length + 3;
  const many = Array.from({ length: count }, (_, i) => ({ id: `s${i}`, kind: "seated" }));
  const tickets = many.map((_, i) => ({ id: `t${i}`, name: `T${i}`, price: count * 100 - i * 100 }));
  const map = Object.fromEntries(many.map((s, i) => [s.id, `t${i}`]));

  const { colorBySectionId, legend } = buildPriceTiers(many, map, tickets);
  assert.equal(legend.length, count);
  const last = PRICE_SCALE[PRICE_SCALE.length - 1].key;
  assert.equal(colorBySectionId[`s${PRICE_SCALE.length - 1}`].key, last);
  assert.equal(colorBySectionId[`s${count - 1}`].key, last);
});
