...
// Spread of a set of numbers, used to assert "these all line up".
...
test("the drill-down straightens a rotated section into a real grid", () => {
  ...
  // Every seat in a row shares a y, and every seat number shares an x. That is
  // only true if the section was un-rotated first — the old version stretched
  // the tilted bounding box and produced a diamond.
...
test("the drill-down scales both axes by the same factor", () => {
  ...
  // Seat pitch across vs row pitch down must keep the ratio they had on the
  // map — fitting each axis independently used to distort it.
  const at = (row, seat) => placed.find((s) => s.rowLabel === row && s.seatLabel === seat);
  const acrossBefore = (30 / 4) * (16 / 10); // one slot, in units
