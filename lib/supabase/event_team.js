...
// single event, either an existing project member added from the roster
// (status 'active') or an outsider invited by email (status 'invited').
...
// inserted row share a UUID. Roster adds are active immediately; email invites
// pass status 'invited' and get an invited_at stamp.
export async function addEventTeamMember(input) {
  ...
  try {
    ...
    if (payload.status === "invited" && !payload.invited_at) {
...
// Re-send a pending invitation — nothing to mail yet, so this just re-stamps
// invited_at so the "Invited …" line reflects the latest send.
export async function resendEventTeamInvite(id) {
  return updateEventTeamMember(id, { invitedAt: new Date().toISOString() });
}
