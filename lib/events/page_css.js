...
// Widest first: each narrower breakpoint is emitted as a diff against the one above it.
const ORDER = BREAKPOINTS.map((b) => b.key);
...
// A nudge is a transform, never a margin, so it can shift a block visually
// without moving anything around it or breaking the flow on a narrow screen.
function offsetDecls(style) {
  const x = Math.round(Number(style?.offsetX) || 0);
  const y = Math.round(Number(style?.offsetY) || 0);
  if (!x && !y) return {};
...
// A transform authored at one breakpoint has to be explicitly cancelled at the
// others, or the diff below silently inherits it down to mobile.
function balanceTransforms(perBp) {
  for (const suffix of Object.keys(perBp.lg)) {
    if (!ORDER.some((bp) => perBp[bp][suffix]?.transform)) continue;
    for (const bp of ORDER) {
      const bucket = perBp[bp][suffix];
      if (bucket && !bucket.transform) bucket.transform = "none";
    }
  }
}
