...
// Distance from the bowl centre to a section's centre, in unit space.
const radius = (s) =>
  Math.hypot((s.x + s.width / 2) * AR - centre.x, s.y + s.height / 2 - centre.y);

// A section as it is actually DRAWN: percent geometry mapped into unit space
// (x scaled by the aspect) and turned by the CSS rotation about its centre.
// Everything below asserts against this, because it is what the buyer sees.
...
// The bug this whole module was rewritten for: blocks were laid out in percent
// space and drawn with a CSS rotation in pixel space, so they piled up.
...
test("a tier's sections are all about the same width", () => {
  ...
  // Spacing by arc length, not by angle, is what keeps an ellipse even.
  const mean = widths.reduce((a, b) => a + b, 0) / widths.length;
  // Blocks span the chord of their own radial wedge, so a little variation on
  // an ellipse is expected; what matters is that none is a sliver.
...
test("a rectangular arena fills its corners rather than leaving four strips", () => {
  ...
  // Straight sides give the four cardinal facings…
  for (const cardinal of [0, 90, 180, 270]) {
    assert.ok(rotations.has(cardinal), `no section faces ${cardinal}`);
  }
  // …and the rounded corners give facings in between.
  assert.ok(
...
// Is this point (in unit space) covered by the block as DRAWN?
...
// Closest approach of two convex quads that don't intersect — how wide the
// walkway between two blocks actually is.
...
// The reason the multiple matters: a real bowl's walkways run straight out from
// the field. Sizing each ring independently staggers the joints instead.
// The point of aligning the aisles: a walkway should run STRAIGHT out from the
// field to the back of the bowl. Sizing each ring independently staggers the
// joints, and the spokes close up somewhere in the middle.
// The reason aisle lines are shared: a walkway should carry on into the next
// ring out. Sizing each ring on its own staggers the joints and the spokes
// close up. (Only ADJACENT tiers are asserted — an oval's aisles follow the
// local normal, not a ray from the centre, so they lean further out with every
// ring, exactly as they do in a real stadium.)
// Aligning the walkways is what makes the gaps read as spokes rather than as
// staggered joints. Every ring is laid out against ONE set of aisle bearings,
// so each tier must hold a whole number of blocks per aisle.
...
test("neighbouring blocks are separated by a real walkway", () => {
  for (const shape of ["oval", "rect"]) {
    ...
    for (const t of ["1", "2"]) {
      // The generator emits each ring in order, so sortOrder neighbours are
      // the blocks either side of a walkway.
...
test("banquet rounds skip the dance floor and stay square", () => {
  ...
  for (const s of drafts) {
    ...
    // Square once drawn, not square in percent — a table is round.
...
test("a bowl too deep for the canvas is squeezed, not spilled", () => {
  ...
  // 30% deep x 5 tiers cannot fit, so the depth must have come down.
...
test("tiers are dropped once squeezing alone would leave them unusable", () => {
  // A field this big leaves room for two rings, not five.
  const plan = planBowl({
...
test("planBowl reports a field no bowl can surround", () => {
  const plan = planBowl({
    // Fills the canvas edge to edge — there is nowhere for a ring to go.
...
// The chairs are drawn in canvas space while the block is drawn with a CSS
// transform; if those two disagree the buyer clicks a block and gets a seat
// grid from somewhere else entirely.
