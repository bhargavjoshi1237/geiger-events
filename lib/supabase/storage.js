...
// Event Wall assets (a logo and self-hosted fonts pulled in by a brand import)
// belong to the project's wall rather than to any one event, so they live in the
// same public bucket under walls/<project-uuid>/.
export function wallMediaPrefix(projectId) {
  return `walls/${projectId}`;
}
...
// Membership plan posters live in the same public bucket under
// memberships/<project-uuid>/. Plans are reusable records, so the poster
// belongs to the project rather than to any one plan row — the plan's config
// stores the public URL (config.posterUrl).
export function membershipMediaPrefix(projectId) {
...
/**
 * Upload one membership plan poster for a project. Compresses to <500 KB by
 * default. Returns `{ path, url }` on success, or `null` on failure. Mirrors
 * uploadBadgeImage: any authenticated caller may write for a project that
 * exists (storage RLS).
 */
export async function uploadMembershipImage(projectId, file, options = {}) {
  if (!projectId || !file || !isSupabaseConfigured()) return null;
  const { compress = true } = options;
  try {
    const payload = compress ? await compressImageUnder(file, 500) : file;
    const path = `${membershipMediaPrefix(projectId)}/${uniqueName(payload)}`;
    const sb = createClient();
    const { error } = await sb.storage
      .from(EVENT_MEDIA_BUCKET)
      .upload(path, payload, {
    ...
    if (error) {
      console.error("[storage.uploadMembershipImage]", error.message);
      return null;
    }
    return { path, url: buildPublicUrl(path) };
  } catch (e) {
    console.error("[storage.uploadMembershipImage]", e);
    return null;
  }
}
/**
...
/**
 * Upload one image for a project's Event Wall (the imported brand mark).
 * Compresses to <500 KB by default — pass `{ compress: false }` for a logo, the
 * same as the event path does. Returns `{ path, url }` on success, or `null` on
 * failure. Mirrors uploadBadgeImage: any authenticated caller may write for a
 * project that exists (storage RLS).
 */
export async function uploadWallImage(projectId, file, options = {}) {
  if (!projectId || !file || !isSupabaseConfigured()) return null;
  const { compress = true } = options;
  try {
    const payload = compress ? await compressImageUnder(file, 500) : file;
    const path = `${wallMediaPrefix(projectId)}/${uniqueName(payload)}`;
    const sb = createClient();
    const { error } = await sb.storage
      .from(EVENT_MEDIA_BUCKET)
      .upload(path, payload, {
    ...
    if (error) {
      console.error("[storage.uploadWallImage]", error.message);
      return null;
    }
    return { path, url: buildPublicUrl(path) };
  } catch (e) {
    console.error("[storage.uploadWallImage]", e);
    return null;
  }
}

/**
 * Upload one video for a project's Event Wall — the hero backdrop, which can be
 * a still or a muted looping clip. Videos are never compressed: they're already
 * encoded and the browser can't re-encode them. Returns `{ path, url }` on
 * success, or `null` on failure. Same storage RLS story as uploadWallImage
 * (any authenticated caller may write for a project that exists).
 */
export async function uploadWallVideo(projectId, file) {
  if (!projectId || !file || !isSupabaseConfigured()) return null;
  try {
    const path = `${wallMediaPrefix(projectId)}/${uniqueName(file)}`;
    const sb = createClient();
    const { error } = await sb.storage
      .from(EVENT_MEDIA_BUCKET)
      .upload(path, file, {
    ...
    if (error) {
      console.error("[storage.uploadWallVideo]", error.message);
      return null;
    }
    return { path, url: buildPublicUrl(path) };
  } catch (e) {
    console.error("[storage.uploadWallVideo]", e);
    return null;
  }
}

/**
 * Upload one self-hosted font file for a project's Event Wall, under
 * walls/<project-uuid>/fonts/. The wall counterpart of uploadEventFont — never
 * compressed, cached hard, `null` on failure so the caller can fall back to
 * hotlinking the source.
 */
export async function uploadWallFont(projectId, file) {
  if (!projectId || !file || !isSupabaseConfigured()) return null;
  try {
    const path = `${wallMediaPrefix(projectId)}/fonts/${uniqueName(file)}`;
    const sb = createClient();
    const { error } = await sb.storage
      .from(EVENT_MEDIA_BUCKET)
      .upload(path, file, {
    ...
    if (error) {
      console.error("[storage.uploadWallFont]", error.message);
      return null;
    }
    return { path, url: buildPublicUrl(path) };
  } catch (e) {
    console.error("[storage.uploadWallFont]", e);
    return null;
  }
}
