...
// Server-only Expo push layer for the portal. Owns events.portal_devices — the
// push tokens registered by the members mobile app. Pure data access + a single
// outbound HTTP call to Expo: validate, console.error on failure, return
// null/false/[]/counts — never throw, never toast.

const EXPO_URL = "https://exp.host/--/api/v2/push/send";
const BATCH_SIZE = 100;

// True for an Expo push token ("ExponentPushToken[...]" / "ExpoPushToken[...]").
...
// Upsert a member's device registration on the unique push_token column. A token
// that moves to another member re-points at the new member_id.
...
// Soft-delete a device registration so it stops receiving pushes.
...
// Distinct live push tokens for the given member ids. Empty input -> [].
...
// Send a notification to a batch of push tokens via Expo's push API. Chunks into
// batches of 100; a "DeviceNotRegistered" ticket soft-deletes the token. Never
// throws — any network error is logged and counted as failed.
