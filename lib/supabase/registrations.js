...
// events.registrations table (and the waitlist/approval RPCs). Pure:
...
// All registrations in a project, newest first. Scoped by the denormalized
// project_id (stamped on every row incl. those created via the public register()
// RPC) so the dashboard shows only the active project's people.
export async function listRegistrations(projectId) {
...
export async function promoteWaitlist(eventId, count = 1) {
  ...
  try {
    const sb = createClient();
    const { data, error } = await sb.rpc("promote_waitlist", {
      p_event_id: eventId,
...
// capacity/approval/waitlist decision to the register RPC and returns a
...
export async function registerForEvent(input = {}) {
  ...
  try {
    ...
    const { data, error } = await sb.rpc("register", {
      ...
      // When false (a registration filed right after a successful buy_ticket),
      // skip the RPC's capacity gate — buy_ticket already enforced it, and
      // re-gating would drop an already-ticketed attendee from the guest list.
      p_enforce_capacity: input.enforceCapacity !== false,
    });
    if (error) {
      console.error("[registrations.register]", error.message);
      // The register RPC raises EVENT_FULL when the event is full and waitlist
      // is disabled — surface it so the caller can say "full" rather than retry.
      const full = /EVENT_FULL/.test(error.message || "");
      return { ok: false, full, error: error.message };
    }
...
// True when this email is already on the event's waitlist. Backs the optional
// "block waitlisted guests from booking again" policy; goes through a
// SECURITY DEFINER RPC so the public (anon) page can check it. Returns false on
// any error or when Supabase isn't configured (fail open — never blocks a sale
// because of an infra hiccup).
export async function hasWaitlistedRegistration(eventId, email) {
  if (!eventId || !email || !isSupabaseConfigured()) return false;
  try {
    const sb = createClient();
    const { data, error } = await sb.rpc("has_waitlisted_registration", {
      p_event_id: eventId,
      p_email: email,
    });
    if (error) {
      console.error("[registrations.hasWaitlisted]", error.message);
      return false;
    }
    return Boolean(data);
  } catch (e) {
    console.error("[registrations.hasWaitlisted]", e);
    return false;
  }
}
