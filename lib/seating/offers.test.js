...
// Two sections, one cheap and one dear, both facing the field from opposite
// sides so the rotation-sensitive paths are exercised.
const SECTIONS = [
  {
    id: "s1",
    name: "101",
...
test("one offer per section and row that has something to sell", () => {
  const offers = build();
  assert.equal(offers.length, 4); // 2 sections x 2 rows
  assert.deepEqual(
...
test("an offer's block is the requested number of ADJACENT seats", () => {
  ...
  for (const offer of offers) {
    ...
    // Adjacent means consecutive seat numbers along the row.
    const nums = offer.seatLabels.map(Number);
    assert.deepEqual(nums, [nums[0], nums[0] + 1, nums[0] + 2], `${offer.seatLabels} not adjacent`);
  }
});

// The bug this guards: a rotated section's raw x ordering scrambles the row, so
// "adjacent" blocks came out as seats scattered across it.
...
test("a taken seat breaks the block and shrinks what's available", () => {
  ...
  // Knock out the middle seat of a six-seat row.
  ...
  // Longest remaining run is 3, so a block of 4 no longer fits.
...
// The reported bug. Every offer in type-first checkout carries the same ticket
// price, so a price sort had nothing to compare and fell through to
// section-then-row — fourteen identical rows of section 104 before anything
// else. Quality is the tie-break that makes the list mean something.
...
// One ticket price is not a range. Rounding it outward invented a slider whose
// only other notch filtered every offer out of its own list.
...
// The bottom notch of the slider has to still return the cheapest seats.
// Flooring the minimum put it BELOW the cheapest offer, so dragging to the end
// of the range emptied the list and showed "No seats match this filter."
