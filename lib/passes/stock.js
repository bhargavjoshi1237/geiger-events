// Physical pass stock and page presets, all in millimetres. One size system
// shared by the renderer, the print sheet, and the raster exporters so what you
// preview is what prints and what downloads.

export const MM_PER_IN = 25.4;

// Dots-per-mm for a raster target (PNG export renders at 300 DPI).
...
// Resolved { wMm, hMm } for a template's stock. A preset wins over the stored
// dimensions unless it's "custom", so switching preset always re-sizes.
...
// How many passes fit on one sheet. Returns at least 1×1 so an oversized pass
// still prints (one per page) instead of producing an empty sheet.
