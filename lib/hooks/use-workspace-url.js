...
export function useWorkspaceUrl() {
  ...
  const apply = useCallback(
    (next) => {
      ...
      put("workflow", next.workflow, null);
  ...
  // Switching workspace tabs exits any open event/section/workflow.
  const setTab = useCallback(
    (next) => apply({ tab: next, event: null, section: null, workflow: null }),
    [apply],
  );
