...
export async function issueRefund({
  ...
  try {
    ...
    // Only a FULL refund returns the seats or booths to the pool — a partial
    // refund (a fee waived, one add-on returned) leaves the buyer where they are.
    if (row.ok && orderTotal > 0 && refundedTotal >= orderTotal) {
      await releaseOrderSeats(orderId);
      await releaseOrderBooths(orderId);
    }
    return {
      ok: Boolean(row.ok),
