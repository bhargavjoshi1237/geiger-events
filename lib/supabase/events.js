...
// `events.events` table. Keeps actions pure: validate, console.error on
...
export function normalizeEvent(row) {
  ...
  return {
    ...
    time: row.event_time ?? "",
    venue: row.venue ?? "",
    // The managed venue this event is held at (null = free-text only). The
    // venue/address/city text above is a denormalized snapshot for display.
    ...
    // The project this event belongs to. Access is scoped to project members
    // (org membership) via RLS — see supabase/sqls/zz_project_access.sql.
    projectId: row.project_id ?? null,
    createdBy: row.created_by ?? null,
    // Whether this event appears on the public Event Wall (lib/supabase/event_wall.js).
    isListable: row.is_listable ?? false,
...
function toRow(input) {
  ...
  const map = {
    ...
    seriesId: "series_id",
    // The owning project (required on create; RLS scopes access to members).
  ...
  if ("isListable" in input) row.is_listable = Boolean(input.isListable);
  return row;
}

// Events in a project, newest first. Requires a project id — without one there
// is nothing to scope to, so we return null (the screen keeps its empty/sample
...
// Events shown on the public Event Wall — listable, not soft-deleted, soonest
// first, scoped to the wall's project. Powers app/w/[slug]/page.js (the wall is
// resolved by slug there, then its project_id scopes this list).
export async function listListableEvents(projectId) {
  if (!projectId || !isSupabaseConfigured()) return null;
  try {
    const sb = createClient();
    const { data, error } = await sb
      ...
      .order("event_date", { ascending: true });
    if (error) {
      console.error("[events.listListable]", error.message);
      return null;
    }
    return (data || []).map(normalizeEvent);
  } catch (e) {
    console.error("[events.listListable]", e);
    return null;
  }
}
...
export async function updateEventMeta(eventId, patch) {
  ...
  try {
    const sb = createClient();
    const { error } = await sb.rpc("event_merge_meta", {
      p_id: eventId,
      p_patch: patch,
    });
    if (error) {
