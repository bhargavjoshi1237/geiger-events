...
//
// Returns { sections, seats } or null. Seats are inserted in chunks because a
// two-tier arena is comfortably tens of thousands of rows.
export async function createBowl(seatMapId, drafts, { replace = false } = {}) {
  ...
  try {
    ...
    for (const section of created) {
      for (const seat of generateSeats(section)) {
        seatRows.push({
