...
// Nudges are compiled to a transform, so they shift a node visually without
// disturbing anything around it. Authored per breakpoint.
...
// Used to badge nudged nodes in the layers list — an offset is invisible in the
// outline otherwise, and it is the easiest thing to forget about on mobile.
export function hasOffset(node) {
  const nudged = (style) => !!(Number(style?.offsetX) || Number(style?.offsetY));
  if (nudged(node?.style)) return true;
  return ["md", "sm"].some((bp) => nudged(node?.[bp]?.style));
}
